...
- **Runs locally** — Everything runs on your laptop. No cloud account required to test the pipeline.
...
- Python 3.11+
- (Optional) [Terraform CLI](https://developer.hashicorp.com/terraform/install) for real deployments
...
✔ Deployment summary written to generated/DEPLOYMENT_SUMMARY.md
...
All fields have sensible defaults — you only need to set `model.path` and `llm.provider` to get started.
...
Create a single file in `backends/`:
...
Edit `core/llm_client.py` and add a new branch in `create_llm()`. Follow the existing pattern for OpenAI/Anthropic/Gemini.
...
python deploy.py --config deploy_config.yaml -v
