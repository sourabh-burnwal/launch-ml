...
async def predict_endpoint(request: Request):
    ...
    try:
        ...
        # Inspect the predict function signature to decide how to call it.
        import inspect
        sig = inspect.signature(predict_fn)
        params = list(sig.parameters.keys())

        if len(params) == 1:
            # Single-arg: pass the first value from the request body,
            # or the whole body if it has a single key.
            first_val = next(iter(body.values())) if len(body) == 1 else body
            result = predict_fn(first_val)
        elif len(params) == 0:
            result = predict_fn()
        else:
            # Multi-arg: try unpacking body as kwargs
